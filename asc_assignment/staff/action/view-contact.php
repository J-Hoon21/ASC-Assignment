...
if (isset($_POST["submit"])) {
    ...
    if ($_FILES["file"]["error"] === UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($fileTmpName, $file_location)) {
            die("<script>alert('File upload failed. Please try again.');window.history.go(-1);</script>");
        }
    } else {
    ...
    $num = intval(substr($row['contact_id'], 3)) + 1;
    $newContactId = "CN" . str_pad($num, 5, "0", STR_PAD_LEFT);
    ...
    $sql = "INSERT INTO contact (contact_id, contact_number, contact_email, contact_subject, contact_message, contact_file, contact_status) 
            VALUES ('$newContactId', '$name', '$email', '$subject', '$message', '$unique_filename', 'pending')";
