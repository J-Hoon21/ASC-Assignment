...
if (isset($_SESSION['customer_id'])) {
    echo ("<script>alert('You Have Log In As Customer!')</script>");
    die("<script>;window.history.go(-1);</script>");
}
if (isset($_SESSION['staff'])) {
    echo ("<script>alert('You Must Logout As Staff First!')</script>");
    die("<script>;window.history.go(-1);</script>");
}
...
$sql = "SELECT customer_id, customer_password FROM customer WHERE customer_email ='$email'";
$login = mysqli_query($conn, $sql);
$customer = mysqli_fetch_assoc($login); //#########USE ' OR 1=1 --' to SQL Injection pentest ##############
...
if (!$customer) {
    ...
    $hashedPassword = $customer['customer_password'];
    if (password_verify($password, $hashedPassword)) {
        ...
        echo ("<script>setTimeout(function() { window.location.href = '../Login Main Page.php'; }, 3000);</script>");
    } else {
